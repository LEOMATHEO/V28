...
let linhas = []; // cada item: {idx, data, prestacao, extraMensal, extraPontual, juros, amort, totalAmort, saldo}
...
function confirmarParcela(){
  ...
  const l = {
    ...
    juros: juros,
    amort: amort,
    totalAmort: amort, // sem extras ainda
    saldo: novoSaldo
  };
  adicionarLinha(l);
...
function exportarCSV(){
  ...
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'ROSELY_v28_lancamentos.csv'; a.click();
}
...
function salvarLocal(){
  localStorage.setItem('rosely_v28', JSON.stringify({
...
function carregarLocal(){
  const raw = localStorage.getItem('rosely_v28');
  if(!raw){ atualizarSaldoOut(parseFloat(document.getElementById('saldo').value)||0); atualizarResumo(); return; }
  try{
